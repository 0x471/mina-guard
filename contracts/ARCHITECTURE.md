...
## On-Chain State (16 Fields)

MinaGuard uses 16 state slots (requires the Mesa 32-slot branch of o1js):
...
| 12 | `delegationKeyHash` | `Poseidon.hashWithPrefix('delegation-key', delegationKey.toFields())` or `Field(0)` (disabled) |
| 13 | `delegationNonce` | Monotonic counter for single-key delegation replay protection |
| 14 | `recipientAllowlistRoot` | MerkleMap root of allowed transfer recipients |
| 15 | `enforceRecipientAllowlist` | `Field(1)` if `executeTransfer` must verify each recipient against the allowlist |
